...
Set a **Signing Secret** in Settings → Webhook Outbound (or in the setup wizard's webhook step; the **Generate** button mints a 64-hex-char value). Copy it into your receiver before saving — the secret is **write-only**: `GET /api/config` never returns it (the API is unauthenticated and may be bound to `0.0.0.0` in Docker), only `webhook.secretConfigured: true|false`. Saving Settings with the secret field left blank keeps the stored value; **Clear** removes it; **Generate** rotates it. The **Test** button signs with whatever is in the secret field right now (blank = the stored secret) and stamps the Instance ID field's current value, so you can verify exactly the headers a save would produce before saving. Once set, every delivery — including test deliveries — carries:
