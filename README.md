...
Current baseline (Apr 2026, Vitest 4, post-DEVX-102 ratchet #5): **~72% lines, ~66% branches, ~80% functions, ~70% statements**. Ratchet #5 covered the setup wizard (`Welcome`, `Auth`, `RecordingsDir`, `Webhook`, `Jira`, `Review` + the `SetupWizard` orchestrator) — including the three auth flows (Chrome auto-detect, EventSource watch, manual token paste) and a full 6-step end-to-end walkthrough — pushing every axis another 6–10 points. Ratchet #4 covered the three major web routes (`Dashboard`, `RecordingDetail`, `Settings`). Ratchet #3 introduced `web/src/test-utils.tsx` (QueryClient + Router + Theme provider harness) and covered every untested web component + the `useTheme` hook. Ratchet #2 covered the server routes cluster (`routes/sync`, `routes/media`, `sync/state`, all 11 endpoints of `routes/recordings`). Ratchet #1 landed the pure-function server cluster + the four Playwright journey specs. Subsequent DEVX-102 PRs continue bumping each axis toward 95% — remaining 0% clusters are server infrastructure (`sync/poller`, `webhook/post`, `auth/chrome-leveldb`, `auth/browser-watch`, `routes/auth`) and `inbox-mcp`.
