...
Current baseline (Apr 2026, Vitest 4, post-DEVX-102 ratchet #4): **~65% lines, ~57% branches, ~70% functions, ~63% statements**. Ratchet #4 covered the three major web routes (`Dashboard`, `RecordingDetail`, `Settings`) — including the transcript parser + Ctrl+F search, audio ref controls, summary modal, URL-synced filter deep-linking, and Zod-flatten error surfacing — pushing every axis 12–16 points. Ratchet #3 introduced `web/src/test-utils.tsx` (QueryClient + Router + Theme provider harness) and covered every untested web component + the `useTheme` hook. Ratchet #2 covered the server routes cluster (`routes/sync`, `routes/media`, `sync/state`, all 11 endpoints of `routes/recordings`). Ratchet #1 landed the pure-function server cluster + the four Playwright journey specs. Subsequent DEVX-102 PRs continue bumping each axis toward 95%.
