...
Current baseline (Apr 2026, Vitest 4, post-DEVX-102 ratchet #7): **~95% lines, ~83% branches, ~93% functions, ~92% statements**. Ratchet #7 is the final tightening: `plaud/client` (0% → 100%), `plaud/transcript` (28% → 100%), `routes/config` gained `/test-webhook` / `/validate-recordings-dir` / `/complete-setup` coverage, `server/src/paths.ts` platform branches, `inbox-mcp/src/db.ts` file-read branches — 68 new tests. Lines / functions / statements sit within a hair of the 95% target; branches remains the laggard at 83% because the hardest-to-hit paths are WSL-only (profiles.ts `wslWindowsUsernames`) and SVG-geometry fallbacks (Waveform.tsx path generation) — DEVX-102 stays open for a small follow-up pass on branches. Earlier ratchets: #6 covered every remaining 0%-covered server-infra file (`sync/poller`, `webhook/post`, `auth/chrome-leveldb`, `auth/browser-watch`, `routes/auth`) plus `inbox-mcp` platform branches. #5 covered the setup wizard. #4 covered the three major web routes (`Dashboard`, `RecordingDetail`, `Settings`). #3 introduced `web/src/test-utils.tsx` and covered every untested web component + the `useTheme` hook. #2 covered the server routes cluster. #1 landed the pure-function server cluster + the four Playwright journey specs.
