...
- Custom headers on every webhook: `User-Agent: rootscribe/0.2.0`, `X-RootScribe-Event: audio_ready|transcript_ready`, and `X-RootScribe-Instance: <instance id>`. When a signing secret is configured, `X-RootScribe-Timestamp` and `X-RootScribe-Signature` are added too — see [Webhook signing](#webhook-signing).
...
Set a **Signing Secret** in Settings → Webhook Outbound (or in the setup wizard's webhook step; the **Generate** button mints a 64-hex-char value). Copy it into your receiver before saving — the secret is **write-only**: `GET /api/config` never returns it (the API is unauthenticated and may be bound to `0.0.0.0` in Docker), only `webhook.secretConfigured: true|false`. Saving Settings with the secret field left blank keeps the stored value; **Clear** removes it; **Generate** rotates it. The **Test** button signs with whatever is in the secret field right now (blank = the stored secret; after **Clear** it sends unsigned, matching what a save would store) and stamps the Instance ID field's current value, so you can verify exactly the headers a save would produce before saving. Once set, every delivery — including test deliveries — carries:
...
The bearer token is stored as plaintext in `settings.json` (with `chmod 600`). The file lives in a user-only directory, and the token's scope is equivalent to "read this user's own Plaud data." OS keychain integration is a future enhancement. The webhook signing secret (`webhook.secret`) and the install's `instanceId` live in the same file; the configuration API (`GET`/`POST /api/config`) never returns the token or the secret. Writes to `settings.json` are atomic (temp file + rename), so an interrupted save never leaves a truncated file. If `settings.json` is present but unparseable (or not a JSON object), the server runs on defaults and does **not** rewrite the file (so a hand-recoverable config is preserved) — the instance id it mints in that state is in-memory only until the file is repaired.
...
- **Fishery for test fixtures.** Data shapes used in 2+ spec files (or with 5+ fields) live as Fishery factories with chainable traits for their common variations — `recordingDetailFactory.reviewed().withJiraLinks(link).build()` instead of a 26-field inline literal. Shared-type factories (`RecordingDetail`, `JiraLink`, `RecordingRow`, `PlaudRawRecording`, `InboxMutationResponse`) live in `shared/src/test-factories/` and import via `@rootscribe/shared/test-factories`. Web-, server-, and inbox-mcp-specific shapes each live in their own package's `src/test-factories/` (or `tests/factories/` for inbox-mcp, whose `SeedRecording` is a test-only type). One-off literals (single use, under 5 fields) stay inline. See [DEVX-104](https://rootfunctionalmedicine.atlassian.net/browse/DEVX-104) for the audit and pattern.
...
Current baseline (Apr 2026, Vitest 4, post-DEVX-102 ratchet #7 final push): **~97% lines, ~87% branches, ~96% functions, ~95% statements**. Three of four axes now clear the 95% target that DEVX-102 opened with; branches plateau at 87% because the remaining uncovered branches are genuinely defensive — optional-chains (`?.`) on values the surrounding code already guarantees, `?? 0` fallbacks on indices the caller clamps in bounds, and `RecordingDetail.tsx`'s auto-scroll math that requires a real browser layout engine (Playwright covers the real scroll path; jsdom reports 0 for `offsetTop`/`scrollTop`/`clientHeight`). Those branches are now marked with `/* v8 ignore */` where obviously unreachable; the remaining 13% are spread across many small UI catches that would benefit more from integration tests than unit tests. Ratchet #7 added 46 tests in this final push on top of the earlier 68 — covering `profiles.ts` WSL `fs` mocks, `chrome-leveldb` scanProfile + decode branches, `routes/auth` parseJwt + validateToken catches, `webhook/post` `readIfExists` EISDIR, `Settings` + `AuthStep` + `Dashboard` + `RecordingDetail` + `SyncStatusBadge` + `SnoozeMenu` remaining branches, `browser-watch` listener-throw, `poller` non-Error branch, `state` rename fast-path, `config.ts` malformed-JSON fallback. Earlier ratchets: #6 covered every remaining 0%-covered server-infra file (`sync/poller`, `webhook/post`, `auth/chrome-leveldb`, `auth/browser-watch`, `routes/auth`) plus `inbox-mcp` platform branches. #5 covered the setup wizard. #4 covered the three major web routes (`Dashboard`, `RecordingDetail`, `Settings`). #3 introduced `web/src/test-utils.tsx` and covered every untested web component + the `useTheme` hook. #2 covered the server routes cluster. #1 landed the pure-function server cluster + the four Playwright journey specs. DEVX-102 closes with this ratchet.
