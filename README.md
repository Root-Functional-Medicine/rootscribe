...
Current baseline (Apr 2026, Vitest 4, post-DEVX-102 ratchet #6): **~89% lines, ~77% branches, ~89% functions, ~87% statements**. Ratchet #6 covered every remaining 0%-covered server-infrastructure file (`sync/poller`, `webhook/post`, `auth/chrome-leveldb`, `auth/browser-watch`, `routes/auth`) plus the `inbox-mcp` platform branches — 90 new tests pushing lines and statements up ~17 points each, functions to ~89%, and branches to ~77%. Ratchet #5 covered the setup wizard (`Welcome`, `Auth`, `RecordingsDir`, `Webhook`, `Jira`, `Review` + the `SetupWizard` orchestrator). Ratchet #4 covered the three major web routes (`Dashboard`, `RecordingDetail`, `Settings`). Ratchet #3 introduced `web/src/test-utils.tsx` (QueryClient + Router + Theme provider harness) and covered every untested web component + the `useTheme` hook. Ratchet #2 covered the server routes cluster (`routes/sync`, `routes/media`, `sync/state`, all 11 endpoints of `routes/recordings`). Ratchet #1 landed the pure-function server cluster + the four Playwright journey specs. One more ratchet gets us to the 95% target — remaining gaps are db-error branches and the few inbox-mcp helpers.
